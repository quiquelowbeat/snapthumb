...
import com.snapthumb.thumbnailgenerator.user_management.application.UserDeleter;
...
public class UserController {
        ...
        public UserController(UserRegister register, UserFinder finder, UserDeleter deleter) {
        ...
        @GetMapping
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Users found successfully (may be empty)", useReturnTypeSchema = true)
        })
        public ResponseEntity<List<UserResponse>> getAll() {
                List<User> users = finder.findAll();
        ...
        public ResponseEntity<Void> deleteUser(@PathVariable String uuid) {
                Either<UserNotFound, Void> deleted = deleter.delete(uuid);
                return deleted.fold(error -> ResponseEntity.notFound().build(),
                                user -> ResponseEntity.noContent().build());
        }
